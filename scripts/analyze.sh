...
# 기존 프로젝트 분석 후 .claude/docs/ 자동 채우기 준비
...
TARGET_DIR="${1:-$(pwd)}"
TODAY=$(date +%Y-%m-%d)
OUTPUT="$TARGET_DIR/.claude/docs/raw-analysis.md"
...
# init.sh로 설치했는지 확인 (settings.json 존재 여부로 판단)
if [ ! -f "$TARGET_DIR/.claude/settings.json" ]; then
  echo "[오류] init.sh가 실행되지 않은 것 같습니다. 먼저 init.sh를 실행하세요."
  exit 1
fi
...
> Claude가 이 파일을 읽고 .claude/docs/ 문서들을 채운 후 이 파일을 삭제합니다.
...
SQL_FILES=$(find "$TARGET_DIR" -name "*.sql" -not -path "*/node_modules/*" -not -path "*/.claude/*" 2>/dev/null | head -10 || true)
if [ -n "$SQL_FILES" ]; then
...
TASK_FILE="$TARGET_DIR/.claude/CURRENT_TASK.md"
...
**[AI 초기 설정] 프로젝트 분석 후 문서 채우기**

analyze.sh가 수집한 raw-analysis.md를 기반으로 아래 문서를 채워주세요.

## 해야 할 것

- [ ] \`.claude/docs/raw-analysis.md\` 읽기
- [ ] \`.claude/docs/architecture.md\` 채우기 (기술 스택, 디렉토리 구조, 데이터 흐름, 외부 의존성)
- [ ] \`.claude/docs/conventions.md\` 채우기 (코딩 패턴, 도메인 규칙, 구현 게이트)
- [ ] \`.claude/docs/raw-analysis.md\` **삭제** (문서 채우기 완료 후)
- [ ] 이 CURRENT_TASK.md를 현재 상태로 업데이트

## 작성 기준

- **추측하지 말 것** — raw-analysis.md에 없는 내용은 \`[확인 필요]\`로 표시
- **현재 상태만** — 미래 계획은 PROGRESS.md에
- 완료 후 "분석 완료, 확인해주세요" 로 보고

## 다음 작업 시작 시 읽어야 할 것

- \`.claude/docs/raw-analysis.md\` (분석 덤프)
- 위 체크리스트
EOF
...
echo ""
echo "생성된 파일:"
echo "  .claude/docs/raw-analysis.md  (프로젝트 정보 덤프)"
echo "  .claude/CURRENT_TASK.md       (Claude 작업 지시)"
echo ""
